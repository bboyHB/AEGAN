...
def Unet_repair_autodata():
    ...
    optimizer_G = SGD(ug.netG.parameters(), lr=config.lr, momentum=0.9, weight_decay=1e-8)
    optimizer_D = SGD(ug.netD.parameters(), lr=config.lr, momentum=0.9, weight_decay=1e-8)
    ...
    for e in range(config.epoch):
        ...
        for i in tqdm(range(iters_per_epoch)):
            ...
            img_G, judge_fake, judge_real = ug(noise_img_defect_tensor, img_origin_tensor)

            recon_loss = criterion_G(img_G, img_origin_tensor)
            lie_loss = criterion_D(judge_fake, target_recon_soft)
