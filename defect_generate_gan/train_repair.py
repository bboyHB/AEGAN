...
from simple_aegan.simple_aegan import SimpleAEGAN
from defect_generate_gan.dg_gan import add_defect
from defect_generate_gan.dg_gan import Generator, Discriminator
...
def train():
    ...
    lr_scheduler_G = torch.optim.lr_scheduler.StepLR(optim_G, 4, 0.1)
    lr_scheduler_D = torch.optim.lr_scheduler.StepLR(optim_D, 4, 0.1)
    criterion_G = torch.nn.MSELoss()
    criterion_D = torch.nn.BCELoss()
    writer = SummaryWriter(log_dir='runs/dfgan'+time_stamp)
    ...
    for e in range(epoch):
        ...
        losses = {'recon_loss': [],
                  ...
                  'detect_loss': [],
                  'real_loss': []}
        for index in tqdm(range(iters_per_epoch)):
            bg_imgs = os.listdir('normal_sample')
            img_numpy = cv2.imread(os.path.join('normal_sample', bg_imgs[random.randint(0, len(bg_imgs) - 1)]))
            img_origin = Image.fromarray(img_numpy[:, :, ::-1]).convert('RGB')
            img_origin_tensor = trans2tensor(img_origin).unsqueeze(0).to(device)
            img_defect = Image.fromarray(add_defect(img_numpy[:, :, ::-1])).convert('RGB')
            ...
            global_step = e * iters_per_epoch + index

            # 训练判别器
            # for repeat in range(2):
            real_prob = model(img_origin_tensor, True)
            ...
            _, recon_prob2 = model(noise_img_defect_tensor)
            ...
            # 训练AE生成器
            # if global_step % 50 == 0:
            recon_img, _ = model(noise_img_defect_tensor)
            recon_loss = criterion_G(recon_img, img_origin_tensor)
            loss_G = recon_loss
            optim_G.zero_grad()
            loss_G.backward()
            optim_G.step()

            _, recon_prob = model(noise_img_defect_tensor)
            ...
            if global_step % show_interval == 0 and global_step != 0:
                recon_img_nonoise, recon_prob_nonoise = model(img_origin_tensor.detach())
                ...
                writer.add_image('common/origin', img_origin_tensor.squeeze(0) / 2 + 0.5, global_step // show_interval)
                writer.add_image('common/noise', noise_img_defect_tensor.squeeze(0) / 2 + 0.5, global_step // show_interval)
